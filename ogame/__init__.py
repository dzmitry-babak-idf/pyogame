...
class OGame(object):
    def __init__(
        ...
        for account in accounts:
            if (
                account["server"]["number"] == self.server_number
        ...
        self.index_php = "https://s{}-{}.ogame.gameforge.com/game/index.php?".format(
            self.server_number, self.language
        )
        ...
        self.landing_page = self.session.get(self.index_php + "page=ingame").text
        self.landing_page = BeautifulSoup4(self.landing_page)

        self.player = self.landing_page.find("meta", {"name": "ogame-player-name"})[
            "content"
        ]
        self.player_id = int(
            self.landing_page.find("meta", {"name": "ogame-player-id"})["content"]
        )
    ...
    def login(self):
        ...
        if response.status_code == 409:
            self.solve_captcha(
                response.headers["gf-challenge-id"].replace(
                    ";https://challenge.gameforge.com", ""
                )
            )
            self.login()
            return True
    ...
    def solve_captcha(self, challenge):
        response = self.session.get(
            url="https://image-drop-challenge.gameforge.com/challenge/{}/en-GB".format(
                challenge
            )
        ).json()
    ...
    def server(self):
        class Server:
            ...
            class Donut:
                galaxy = self.landing_page.find("meta", {"name": "ogame-donut-galaxy"})[
                    "content"
                ]
                if 1 == int(galaxy):
                    galaxy = True
                    ...
                    galaxy = False
                system = self.landing_page.find("meta", {"name": "ogame-donut-system"})[
                    "content"
                ]
                if 1 == int(system):
                    system = True
    ...
    def planet_names(self):
        return [
            planet.text for planet in self.landing_page.find_all(class_="planet-name")
        ]
    ...
    def name_by_planet_id(self, id):
        for _id, planet_name in zip(OGame.planet_ids(self), OGame.planet_names(self)):
            if id == _id:
                return planet_name
    ...
    def slot_celestial(self):
        class Slot:
            planets = (
                self.landing_page.find("p", attrs={"class": "textCenter"})
                .find("span")
                .text.split("/")
            )
    ...
    def celestial(self, id):
        response = self.session.get(
            url=self.index_php + "page=ingame&component=overview", params={"cp": id}
        ).text
        textContent1 = re.search(
            r'textContent\[1] = "(.*)km \(<span>(.*)<(.*)<span>(.*)<', response
        )
        textContent3 = re.search(r'textContent\[3] = "(.*)"', response)
        textContent3 = textContent3.group(1).replace("\\u00b0", "")
        textContent3 = re.findall(r"\d+(?: \d+)?", textContent3)
        textContent7 = re.search(
            r'textContent\[7] = "(.*)>(.*?) \(Place (.*?) (.*)<', response
        )
        ...
        class Celestial:
            ...
            free = total - used
            temperature = [textContent3[0], textContent3[1]]
    ...
    def celestial_queue(self, id):
        ...
        class Queue:
            ...
            list = [research, buildings, shipyard]
    ...
    def resources(self, id):
        ...
        class Resources:
            resources = [
                bs4.find(id="resources_metal")["data-raw"],
            ...
            day_production = bs4.find("tr", attrs={"class": "summary"}).find_all(
                "td", attrs={"class": "undermark"}
            )
            day_production = [
    ...
    def resources_settings(self, id, settings=None):
        ...
        names = ["last1", "last2", "last3", "last4", "last12", "last212", "last217"]
        ...
        if settings is not None:
            for building, speed in settings.items():
                settings_form.update({"last{}".format(building[0]): speed * 10})
            self.session.post(
                self.index_php + "page=resourceSettings&cp={}".format(id),
                data=settings_form,
            )
    ...
    def supply(self, id):
        response = self.session.get(
            url=self.index_php + "page=ingame&component=supplies&cp={}".format(id)
        ).text
        bs4 = BeautifulSoup4(response)
        levels = [
    ...
    def facilities(self, id):
        response = self.session.get(
            self.index_php + "page=ingame&component=facilities&cp={}".format(id)
        ).text
        bs4 = BeautifulSoup4(response)
        levels = [
            int(level["data-value"])
            for level in bs4.find_all("span", {"class": "level", "data-value": True})
        ]
        technologyStatus = [
            status["data-status"]
            for status in bs4.find_all("li", {"class": "technology"})
    ...
    def lf_facilities(self, id):
        response = self.session.get(
            self.index_php + "page=ingame&component=lfbuildings&cp={}".format(id)
        ).text
        bs4 = BeautifulSoup4(response)
        levels = [
            int(level["data-value"])
    ...
    def moon_facilities(self, id):
        response = self.session.get(
            url="{}page=ingame&component=facilities&cp={}".format(self.index_php, id)
        ).text
        bs4 = BeautifulSoup4(response)
        levels = [
            int(level["data-value"])
            for level in bs4.find_all(class_=["targetlevel", "level"])
            if level.get("data-value")
        ]
        technologyStatus = [
            status["data-status"]
            for status in bs4.find_all("li", {"class": "technology"})
    ...
    def research(self, id=None):
        ...
        response = self.session.get(
            url=self.index_php,
            params={"page": "ingame", "component": "research", "cp": id},
        ).text
        bs4 = BeautifulSoup4(response)
        levels = [
            int(level["data-value"])
    ...
    def lf_research_humans(self, id=None):
        ...
        response = self.session.get(
            url=self.index_php,
            params={"page": "ingame", "component": "lfresearch", "cp": id},
        ).text
        bs4 = BeautifulSoup4(response)
        ...
        class LfResearch:
            def __init__(self, i):
                if (
                    i
                    <= technology_status.count("on")
                    + technology_status.count("disabled")
                    - 1
                ):
                    self.level = levels[i]
    ...
    def ships(self, id):
        response = self.session.get(
            self.index_php + "page=ingame&component=shipyard&cp={}".format(id)
        ).text
        bs4 = BeautifulSoup4(response)
        ships_amount = [
            int(level["data-value"]) for level in bs4.find_all(class_="amount")
        ]
        technologyStatus = [
            status["data-status"]
            for status in bs4.find_all("li", {"class": "technology"})
        ...
        class Crawler:
            if id not in OGame.moon_ids(self):
                ...
                self.in_construction = OGame.inConstruction(technologyStatus[16])
            else:
                amount = 0
                is_possible = False
    ...
    def defences(self, id):
        response = self.session.get(
            self.index_php + "page=ingame&component=defenses&cp={}".format(id)
        ).text
        bs4 = BeautifulSoup4(response)
        defences_amount = [
            int(level["data-value"]) for level in bs4.find_all(class_="amount")
    ...
    def galaxy(self, coords):
        ...
        player_name = {
            playerId(player["id"]): player.h1.span.text for player in players
        }
        player_rank = {
            playerId(player["id"]): int(player.a.text)
            for player in players
            if player.a.text.isdigit()
        }

        alliances = bs4.find_all_partial(id="alliance")
        alliance_name = {
            playerId(alliance["id"]): alliance.h1.text.strip() for alliance in alliances
        }
        ...
        for row in bs4.select("#galaxytable .row"):
            ...
            if "empty_filter" in status:
                ...
                if (
                    re.search(
                        ...
                        str(row.find(class_=re.compile(r"status_abbr_outlaw tooltip"))),
                    )
                    is not None
                ):
            ...
            alliance_id = playerId(alliance_id["rel"]) if alliance_id else None
            ...
            if len(activity_tag) != 0:
                if "minute15" in activity_tag[0]["class"]:
                    ...
                    flag_str = row.findAll("div", {"title": "Activity"})[0].string
                    # if showMinutes, set as real count
                    flag_activity = int(re.search(r"[0-9]+", flag_str).group())
                else:
    ...
    def galaxy_debris(self, coords):
        ...
        for row in debris_rows:
            ...
            debris_cord = const.coordinates(
                coords[0], coords[1], int(debris_cord), const.destination.debris
            )
            ...
            class Position:
                ...
                crystal = resources[1]
                deuterium = resources[2]
                list = [position, has_debris, resources, metal, crystal, deuterium]
    ...
    def fleet_coordinates(self, event, Coords):
        ...
        destin = (
            Coords.replace("destCoords", "destFleet")
            .replace("coordsOrigin", "originFleet")
            .replace("destinationCoords", "destinationData")
            .replace("originCoords", "originData")
        )  # for hostile/friendly_fleets
        destination = [
            dest.find(class_=destin).find("figure", {"class": "planetIcon"})
            if dest.find(class_=destin).find("figure", {"class": "planetIcon"})
            is not None
            else BeautifulSoup4('<figure class="planetIcon planet"></figure>').find(
                "figure"
            )
            for dest in event
        ]
        destination = [
            const.convert_to_destinations(dest["class"]) for dest in destination
        ]
        coordinates = []
    ...
    def slot_fleet(self):
        ...
        slots = [slot.text for slot in slots.find_all(class_="fleft")]
        fleet = re.search(":(.*)/(.*)", slots[0])
        fleet = [fleet.group(1), fleet.group(2)]
        expedition = re.search(" (.*)/(.*)\\n", slots[1])
        expedition = [expedition.group(1).replace(" ", ""), expedition.group(2)]
    ...
    def friendly_fleet(self):
        ...
        arrival_times = [
            datetime.fromtimestamp(timestamp) for timestamp in arrival_times
        ]
    ...
    def hostile_fleet(self):
        ...
        fleet_ids = [re.search("eventRow-(.*)", id).group(1) for id in fleet_ids]

        mission_types = [int(event["data-mission-type"]) for event in eventFleet]
        ...
        player_names = [
            name.find_all("td", class_="sendMail")[0].a["title"] for name in eventFleet
        ]
    ...
    def jump_fleet(self, origin_id, target_id, ships):
        self.session.get(
            url="{}page=ingame&component=facilities&cp={}".format(
                self.index_php, origin_id
            )
        )
        ...
        if str(target_id) not in possible_dest:
            ...
            return False
        form_data = {"token": jump_token, "zm": target_id}
        for ship in ships:
    ...
    def phalanx(self, coordinates, id):
        raise NotImplemented("Phalanx get you banned if used with invalid parameters")
    ...
    def get_messages(self):
        ...
        for data in messages:
            ...
            if data["unreadCounter"] >= 1:
                ...
                chat_history = [
                    [authors[count], message] for count, message in enumerate(messages)
                ]
            ...
            class Message:
                ...
                chat = chat_history
                list = [player, player_id, status, text, time, alliance, rank, chat]

            message_data.append(Message)
        return message_data

    def rename_planet(self, id, new_name):
        self.session.get(url=self.index_php, params={"cp": id})
        response = self.session.get(
            url=self.index_php,
            params={"page": "planetlayer"},
            headers={
                "Referer": f"{self.index_php}page=ingame" f"&component=overview&cp={id}"
            },
        ).text
        token_rename = re.search("name='token' value='(.*)'", response).group(1)
        param = {"page": "planetRename"}
        data = {"newPlanetName": new_name, "token": token_rename}
        response = self.session.post(
            url=self.index_php,
            params=param,
            data=data,
    ...
    def abandon_planet(self, id):
        ...
        response = self.session.get(
            self.index_php, params={"page": "planetlayer"}, headers=header
        ).text
        response = response[
            response.find('input type="hidden" name="abandon" value="') :
        ]
        ...
        for link in report_links:
            ...
            for resource in resources_list.find_all("li"):
                ...
                resources_data[resource_name[0]] = int(
                    resource["title"].replace(".", "").replace(",", "")
                )
            ...
            def get_tech_and_quantity(tech_type):
                ...
                if "unable" in tech_list.text:
                    ...
                    for tech in tech_list.find_all("li", {"class": "detail_list_el"}):
                        tech_id = int(
                            re.search(r"([0-9]+)", tech.find("img")["class"][0]).group(
                                1
                            )
                        )
                        tech_amount = int(
                            tech.find("span", "fright")
                            .text.replace(".", "")
                            .replace(",", "")
                        )
            ...
            for tech_type in spied_data.keys():
                for tech_id, tech_amount in get_tech_and_quantity(tech_type):
                    if tech_id == None:
                        spied_data[
                            tech_type
                        ] = "detail_list_fail"  # replace dict with message string
                        break
    ...
    def extract_other_message(self, response):
        ...
        for msg_raw in msgs_raw:
            ...
            if "msg_date" in str(msg_raw):
                message_date = str(
                    msg_raw.select_one(
                        "li.msg div.msg_head span.fright span.msg_date"
                    ).text
                )
                msgs.append(message_date)
            if "msg_sender" in str(msg_raw):
                message_sender = str(
                    msg_raw.select_one("li.msg div.msg_head span.msg_sender").text
                )
                msgs.append(message_sender)
            if "msg_content" in str(msg_raw):
                message_text = str(
                    msg_raw.select_one("li.msg span.msg_content").text.strip()
                )
    ...
    def extract_combat_summary_reports_message(self, response):
        ...
        for msg_raw in msgs_raw:
            ...
            message_destination = const.coordinates(
                int(message_destination[0]),
                int(message_destination[1]),
                int(message_destination[2]),
                int(message_destination_type),
            )
            ...
            if not fleet_lost_first_round_message:
                ...
                debris_field_title = msg_raw.select(
                    "span.msg_content div.combatLeftSide span"
                )[2].attrs["title"]
                ...
                link = str(
                    msg_raw.select_one("div.msg_actions a span.icon_attack").parent
                )
                link = link.replace("&amp;", "&")
                re_link = re.search(
                ...
                re_link = const.coordinates(
                    int(re_link[1]), int(re_link[2]), int(re_link[3]), int(re_link[4])
                )
    ...
    def get_new_messages_count(self):
        ...
        response = self.session.get(
            url=self.index_php + "page=messages", params=payload
        ).text
        bs4 = BeautifulSoup4(response)
    ...
    def send_fleet(
        self, mission, id, where, ships, resources=(0, 0, 0), speed=10, holdingtime=0
    ):
        response = self.session.get(
            url=self.index_php + "page=ingame&component=fleetdispatch&cp={}".format(id)
        ).text
    ...
    def return_fleet(self, fleet_id):
        ...
        if "return={}".format(fleet_id) in response:
            token = (
                re.search("return={}".format(fleet_id) + '&amp;token=(.*)" ', response)
                .group(1)
                .split('"')[0]
            )
            self.session.get(
                url="".join(
                    [
                        self.index_php,
                        "page=ingame&component=movement&return={}&token={}".format(
                            fleet_id, token
                        ),
                    ]
                )
    ...
    def build(self, what, id):
        ...
        component = what[2]
        response = self.session.get(
            url=self.index_php + "page=ingame&component={}&cp={}".format(component, id)
        ).text
    ...
    def deconstruct(self, what, id):
        ...
        deconstruct_token = re.search(
            r"var downgradeEndpoint = (.*)token=(.*)\&", response
        ).group(2)
        self.session.get(
            url=self.index_php,
            params={
    ...
    def cancel(self, what_queue, id):
        ...
        cancel_token = re.search(
            rf"var cancelLink{what_queue} = (.*)token=(.*)\&", response
        ).group(2)
        parameters = re.search(rf"\"cancel{what_queue}\((.*)\, (.*)\,", response)
    ...
    def relogin(self, universe=None):
        ...
        OGame.__init__(
            self, universe, self.username, self.password, self.user_agent, self.proxy
        )
